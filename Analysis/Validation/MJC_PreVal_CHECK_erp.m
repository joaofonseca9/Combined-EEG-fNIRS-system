...
for iSub = 1:length(subject)
    ...
    file.results     = fullfile([mainpath_out,'sub-',sub,'\ResultsMatrix\sub-',sub,'_','checkVEP.mat']);
    file.results_bad = fullfile([mainpath_out,'sub-',sub,'\ResultsMatrix\sub-',sub,'_','checkVEP_bad.mat']);
%     file.preprocessed = fullfile(mainpath_in,['sub-',sub],'eeg',['sub-',sub,'_rec-',rec,'_eeg_preprocessed.mat']);
    %% Load EEG data
...
%     [EEG]  = pop_loadset(['sub-',sub,'_rec-',rec,'_eeg_filtData.set'],fullfile(sub_path,'eeg'));
    EEG_check=extractCheckerboardTask(EEG);
    
    %% Re reference to M2
    locs = {EEG_check.chanlocs.labels};
    M2_loc = find(contains(locs, 'M2'));
    [EEG] = pop_reref(EEG, M2_loc);
    
    [ALLEEG, EEG, CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname','eegdata','gui','off');
...
function [EEG_letter]=extractCheckerboardTask(EEG)
event_samp  = [EEG.event.latency];

%CHECK TRIALS
CHECK_start=event_samp((strcmp({EEG.event.type}, 's1500'))==1);
CHECK_stop=event_samp((strcmp({EEG.event.type}, 's1555'))==1);

[EEG_letter] = eeg_eegrej(EEG, [CHECK_start-floor(0.5*EEG.srate) CHECK_stop+ceil(0.5*EEG.srate)] );
end
...
% create time vector in seconds
    ...
    event_samp  = [EEG.event.latency];
    event_check  = event_samp((strcmp({EEG.event.type}, 's1255'))==1);
    
    % extract trials ERP
    for iTrial = 1:length(event_check)
        data(:,:,iTrial) = EEG.data(:, [event_check(iTrial)-floor(0.1*EEG.srate) : event_check(iTrial)+ceil(0.4*EEG.srate)]);
    end
