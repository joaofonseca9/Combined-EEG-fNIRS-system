%% VAL_CHECK_erp
% Evaluate pattern-reversal visual evoked potenial (VEP) evoked by
...
%     mainpath_out     = 'C:\Users\joaop\OneDrive - Universidade do Porto\Erasmus\Internship\Experiment\Data\Exp\processed\';
    mainpath_in = 'C:\Users\joaop\OneDrive - Universidade do Porto\Erasmus\Internship\Experiment\Data\Validation Data\EEG';
    file.CHECK       = fullfile(mainpath_in,['ID',ID,'_gelEEG_CHECK.mat']);
    file.results     = fullfile(mainpath_in,['ID',ID,'checkVEP_eegonly.mat']);
    file.results_bad     = fullfile(mainpath_in,['ID',ID,'checkVEP_bad_eegonly.mat']);
    
    %% Load EEG data

    % add paths to subject folders and files
    sub_path = fullfile([mainpath_out,'ID',ID, '\']);
    addpath(sub_path);

    % load EEG-files and create a new dataset in eeglab
    load(file.CHECK, 'EEG_CHECK');
    [ALLEEG, EEG, CURRENTSET] = pop_newset(ALLEEG, EEG_CHECK, 1,'setname','eegdata','gui','off');
    ...
    % check for correct number of trials (300)
    if strcmp(ID,'18')==1 && strcmp(cap,'gel')==1
        data(:,:,[1:(size(data,3)-300)]) = [];
    elseif size(data,3)~=300
        error('INCORRECT NUMBER OF TRIALS: %d',size(data,3));
    end
    ...
    %% Save structure for Checkerboard ERP

    CHECKerp.data      = data;
    CHECKerp.ERP       = ERP;
    CHECKerp.GFPt      = GFPt; 
    CHECKerp.time      = time;
    CHECKerp.chans     = {EEG.chanlocs.labels};
    
    CHECKerpBAD.chans     = {EEG.chanlocs.labels};
    CHECKerpBAD.badchans  = EEG.badchan;
    CHECKerpBAD.badtrial  = badtrial;

    save(file.results,'CHECKerp');
...
% create time vector in seconds
    ...
    event_flip  = event_samp((strcmp({EEG.event.type}, 's1255'))==1);
    
    % extract trials ERP
    for iTrial = 1:length(event_flip)
        data(:,:,iTrial) = EEG.data(:, [event_flip(iTrial)-floor(0.1*EEG.srate) : event_flip(iTrial)+ceil(0.4*EEG.srate)]);
    end
