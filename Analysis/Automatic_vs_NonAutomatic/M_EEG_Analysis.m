...
for trial=1:10
    ...
    power_theta_allTrials(:, trial) = power_theta;
    power_alpha_allTrials(:, trial) = power_alpha;
    power_beta_allTrials(:, trial) = power_beta;
    
    power_theta_final = mean(power_theta_allTrials, 2);
    power_alpha_final = mean(power_alpha_allTrials, 2);
    power_beta_final = mean(power_beta_allTrials, 2);
end
...
topoplot(power_theta_final, EEG_AutoUncued.chanlocs, 'electrodes', 'on');
colorbar;
subplot(1, 3, 2);
text(-0.13, 0.7, 'Alpha', 'FontSize', 18)
topoplot(power_alpha_final, EEG_AutoUncued.chanlocs, 'electrodes', 'on');
colorbar;
subplot(1, 3, 3);
text(-0.1, 0.7, 'Beta', 'FontSize', 18)
topoplot(power_beta_final, EEG_AutoUncued.chanlocs, 'electrodes', 'on');
colorbar;   
